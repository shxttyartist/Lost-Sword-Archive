  ...
  items.forEach(item => {
	...
	itemDiv.querySelector('.buy-btn').addEventListener('click', () => {
		...
		if (happyCoins >= item.price) {
			...
			imgEl.outerHTML = `<a href="${item.unlockedSrc.replace('mini','full')}" download><img id="${item.id}-img" src="${item.unlockedSrc}"></a>`;


			itemDiv.querySelector('.lock-overlay').style.display = 'none';
